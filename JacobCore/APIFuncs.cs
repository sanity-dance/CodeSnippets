...
namespace JacobCore
{
    class APIFuncs
    {
        ...
        /// </summary>
        /// <param name="requestInfo">Array of string in the format ["VERB", "requestUrl"]</param>
        ...
        public static async Task<(bool, JObject)> InvokeAPI(string[] requestInfo, string accessToken, bool overrideAuthorizationHeader = false, string stringContent = null, byte[] byteContent = null)
        {
            using (var request = new HttpRequestMessage(new HttpMethod(requestInfo[0]), requestInfo[1]))
            {
                ...
                var contentJson = JObject.Parse(string.IsNullOrWhiteSpace(contentString) ? "{}" : contentString);
        ...
        public static async Task<bool> GetIterativeContent(string initialUri, Action<JObject> action, string accessToken, string failureMessage)
        {
            ...
            {
                itemsReturned = 0;
                string[] currentRequest = { "GET", currentUrl };
